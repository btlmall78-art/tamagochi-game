...
function openStartScreen(mode = 'create') {
  ...
  document.querySelectorAll('.top-bar, .panel-tabs, .level-box, .stats-row, .rider-container, .game-actions, .game-title, .footer-note, .mini-game').forEach(el => {
...
function syncPanelTabs(activePanelId = null) {
  document.querySelectorAll('.panel-tab-btn').forEach(btn => {
    const isActive = btn.dataset.panelTarget === activePanelId;
...
function setupPanelTabs() {
  document.querySelectorAll('.panel-tab-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (target === 'inventoryModal') {
        openInventoryModal(true);
        return;
      }

      if (target === 'workshopPanel') {
        handleWorkshop();
        return;
      }

      if (target === 'questsPanel') {
        handleQuests();
        return;
      }

      if (target === 'achievementsPanel') {
        handleAchievements();
        return;
      }
    });
...
function setupMainEvents() {
  ...
  if (trickBtn) trickBtn.addEventListener('click', handleTrickClick);
  if (rideBtn) rideBtn.addEventListener('click', handleRideClick);
