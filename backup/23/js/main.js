...
const shopItems = [
  ...
  },
  {
    ...
    price: 34,
    bonus: {},
    desc: 'Всё нужное всегда с собой'
  },
  {
    id: 'gloves',
    type: 'clothes',
  ...
  },
  {
    ...
    price: 10,
    effect: { energy: 15 },
    desc: 'Быстрый перекус и +15 энергии'
  },
  {
    id: 'isotonic',
    type: 'consumable',
...
function getInventoryEntries() {
  const ownedItems = Array.from(new Set(player.ownedShopItems || []))
  ...
  const consumableItems = (player.consumables || [])
    .map(entry => {
  ...
  return [...ownedItems, ...consumableItems];
}

function getFilteredInventoryEntries(entries) {
...
function syncSelectedInventoryItem(allEntries, filteredEntries) {
  const preferredList = filteredEntries.length ? filteredEntries : allEntries;
  ...
  if (selectedInventoryItem) {
    const stillExists = preferredList.find(entry => entry.key === selectedInventoryItem.key);
    if (stillExists) {
      selectedInventoryItem = stillExists;
      return;
    }
  }

  selectedInventoryItem = preferredList[0];
}
...
function getInventoryVehicleEmoji() {
  if (player.type === 'skate') return '🛹';
  if (player.type === 'roller') return '🛼';
  return '🛴';
}
...
function renderInventoryRiderPreview() {
  ...
  if (player.equipped?.head) {
    const item = shopItems.find(i => i.id === player.equipped.head);
    if (item) equippedItems.push(`<span class="inventory-rider-chip">${item.emoji} ${item.name}</span>`);
  }

  if (player.equipped?.body) {
    const item = shopItems.find(i => i.id === player.equipped.body);
    if (item) equippedItems.push(`<span class="inventory-rider-chip">${item.emoji} ${item.name}</span>`);
  }

  if (player.equipped?.special) {
    const item = shopItems.find(i => i.id === player.equipped.special);
    if (item) equippedItems.push(`<span class="inventory-rider-chip">${item.emoji} ${item.name}</span>`);
  }

  preview.innerHTML = `
    <div class="inventory-rider-stage">
      <div class="inventory-rider-avatar">${player.skin || '🧑'}</div>
      <div class="inventory-rider-vehicle">${getInventoryVehicleEmoji()}</div>
    </div>
    <div class="inventory-rider-equipped">${equippedItems.length ? equippedItems.join('') : '<span class="inventory-rider-chip muted">НЕТ ЭКИПИРОВКИ</span>'}</div>
  `;
}

function renderInventorySelectedItemInfo() {
  const info = document.getElementById('inventorySelectedItemInfo');
  if (!info) return;

  if (!selectedInventoryItem) {
    info.innerHTML = '<div class="inventory-selected-empty">ВЫБЕРИ ПРЕДМЕТ</div>';
    return;
  }

  const { item, kind, count, equipped } = selectedInventoryItem;
  const safeDesc = item.desc || '• Без описания •';

  if (kind === 'consumable') {
    info.innerHTML = `
      <div class="inventory-selected-card">
        <div class="inventory-selected-title inventory-selected-title-only">${item.name}</div>
        <div class="inventory-selected-desc">• ${safeDesc} •</div>
        <div class="inventory-selected-bottom-row">
          <div class="inventory-selected-count">x${count}</div>
  ...
  info.querySelector('[data-toggle-id]')?.addEventListener('click', event => {
    const selectedId = event.currentTarget.dataset.toggleId;
    const selectedItem = shopItems.find(i => i.id === selectedId);
    if (!selectedItem) return;

    const isEquipped = player.equipped && player.equipped[selectedItem.category] === selectedId;
    if (isEquipped) {
      unequipShopItem(selectedItem.category);
    } else {
      equipInventoryItem(selectedId);
    }
  });

  info.querySelector('[data-use-id]')?.addEventListener('click', event => {
    useConsumableItem(event.currentTarget.dataset.useId);
  });
}

function createInventoryCard(entry) {
  const div = document.createElement('div');
  const isSelected = selectedInventoryItem && selectedInventoryItem.key === entry.key;

  div.className = 'inventory-card' + (isSelected ? ' selected' : '') + (entry.equipped ? ' is-equipped' : '');
  div.dataset.selectId = entry.key;

  div.innerHTML = `
    <div class="inventory-card-title">${entry.item.name}</div>
    <div class="inventory-icon">${entry.item.emoji}</div>
    <div class="inventory-card-footer">
      ${entry.kind === 'consumable'
        ? `<span class="inventory-badge inventory-count-badge">x${entry.count}</span>`
        : (entry.equipped ? '<span class="inventory-badge">НАДЕТО</span>' : '<span class="inventory-card-dot">•</span>')}
    </div>
  `;
  ...
  div.addEventListener('click', () => {
    ...
    renderInventorySelectedItemInfo();
    document.querySelectorAll('#inventoryModalContent .inventory-card').forEach(card => {
      card.classList.toggle('selected', card === div);
    });
  });
...
function renderInventoryModal() {
  ...
  try {
    ...
    const filteredEntries = getFilteredInventoryEntries(allEntries);

    syncSelectedInventoryItem(allEntries, filteredEntries);
    renderInventoryRiderPreview();
    renderInventorySelectedItemInfo();
    ...
    filteredEntries.forEach(entry => {
      content.appendChild(createInventoryCard(entry));
    });
  } catch (error) {
