...
function startTrickRound() {
  ...
  const w = trickMiniGame.offsetWidth;
  const rw = trickRunner.offsetWidth;
  if (w === 0 || rw === 0) return;

  const greenW = 80 + Math.random() * 140;
  const greenL = Math.random() * (w - greenW);

  trickGreenZone.style.left = `${greenL}px`;
  trickGreenZone.style.width = `${greenW}px`;
  ...
  runnerInterval = setInterval(() => {
    ...
    trickRunner.style.left = `${Math.max(0, Math.min(pos, w - rw))}px`;
  }, 16);
}
...
trickMiniGame.addEventListener('click', () => {
  if (!isTrickOverlayOpen()) return;
  if (!runnerInterval || !trickRoundActive) return;

  clearInterval(runnerInterval);
  runnerInterval = null;
  trickRoundActive = false;

  const runnerPos = parseInt(trickRunner.style.left || '0', 10) + trickRunner.offsetWidth / 2;
  const greenL = parseFloat(trickGreenZone.style.left) || 0;
  const greenW = parseFloat(trickGreenZone.style.width) || 0;
  ...
  if (success) {
    ...
    if (trickResultText) {
      trickResultText.textContent = `✨ УСПЕХ! КОМБО x${player.combo}`;
    }
  } else {
    ...
    if (trickResultText) {
      trickResultText.textContent = `💥 ПРОМАХ -${BALANCE.penaltyCoins}💰`;
    }
  }
