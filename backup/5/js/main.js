...
function renderShopModal() {
  ...
  shopItems.forEach(item => {
    ...
    if (currentShopFilter === 'clothes' && item.type !== 'clothes') return;
    if (currentShopFilter === 'consumables' && item.type !== 'consumable') return;
  ...
  try {
    ...
    if (ownedClothes.length === 0 && consumables.length === 0) {
      ...
      ownedClothes.forEach(item => {
        if (currentInventoryFilter === 'consumables') return;
        ...
        div.innerHTML = `
          ...
          <div class="inventory-actions">
            ${equipped
              ...
              `
              : `<button class="inventory-btn" data-id="${item.id}" data-cat="${item.category}">НАДЕТЬ</button>`}
          </div>
        `;
      ...
      consumables.forEach(entry => {
        if (currentInventoryFilter === 'clothes') return;
      ...
      content.querySelectorAll('[data-unequip-cat]').forEach(btn => {
        btn.addEventListener('click', e => {
          const button = e.currentTarget;
          unequipShopItem(button.dataset.unequipCat);
        });
      });
    ...
    console.error('Ошибка в openInventoryModal:', error);
