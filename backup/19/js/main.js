...
function getInventoryEntries() {
  const clothesEntries = (player.ownedShopItems || [])
  ...
  const consumableEntries = (player.consumables || [])
    .map(entry => {
  ...
  return [...clothesEntries, ...consumableEntries];
}

function getFilteredInventoryEntries() {
  const entries = getInventoryEntries();

  if (currentInventoryFilter === 'clothes') {
    return entries.filter(entry => entry.kind === 'clothes');
  }

  if (currentInventoryFilter === 'equipped') {
    return entries.filter(entry => entry.kind === 'clothes' && entry.equipped);
  }

  if (currentInventoryFilter === 'consumable') {
    return entries.filter(entry => entry.kind === 'consumable');
  }

  return entries;
}

function updateInventoryStickyOffsets() {
  const panelBody = document.querySelector('#inventoryModal .inventory-panel-body');
  const stickyTop = document.getElementById('inventoryStickyTop');
  if (!panelBody || !stickyTop) return;

  panelBody.style.setProperty('--inventory-sticky-offset', `${stickyTop.offsetHeight + 8}px`);
}
...
function renderInventoryRiderPreview() {
  ...
  const skinUrl = getSkinImageUrl(player.type || 'scooter', player.skin || `${player.type || 'scooter'}1`);

  const equippedHead = player.equipped?.head
    ? shopItems.find(i => i.id === player.equipped.head)
    : null;
  const equippedBody = player.equipped?.body
    ? shopItems.find(i => i.id === player.equipped.body)
    : null;

  preview.innerHTML = `
    <div class="inventory-rider-stage">
      <div class="inventory-rider-equipment inventory-rider-equipment-top">
        ${equippedHead ? `<span class="inventory-rider-chip">${equippedHead.emoji} ${equippedHead.name}</span>` : ''}
      </div>

      <div class="inventory-rider-avatar-wrap">
        <img
          class="inventory-rider-avatar"
          src="${skinUrl}"
          alt="${riderName}"
          onerror="this.onerror=null;this.src='${getFallbackDataUri('RIDER')}';"
        >
      </div>

      <div class="inventory-rider-equipment inventory-rider-equipment-bottom">
        ${equippedBody ? `<span class="inventory-rider-chip">${equippedBody.emoji} ${equippedBody.name}</span>` : ''}
      </div>
    </div>
  `;
}
...
function renderInventorySelectedItem() {
  ...
  if (!selectedInventoryItem) {
    info.innerHTML = `
      <div class="inventory-selected-item-empty">
        Выбери предмет ниже
      </div>
    `;
    return;
  ...
  const typeText = kind === 'consumable'
    ? 'РАСХОДНИК'
    : equipped
      ? 'ЭКИПИРОВАНО'
      : 'ОДЕЖДА';

  const statusText = kind === 'consumable'
    ? `x${count}`
    : equipped
      ? 'НАДЕТО'
      : 'В ИНВЕНТАРЕ';
  ...
  info.innerHTML = `
    <div class="inventory-selected-item-card">
      <div class="inventory-selected-item-top">
        <div class="inventory-selected-item-type">${typeText}</div>
        <div class="inventory-selected-item-status">${statusText}</div>
      </div>

      <div class="inventory-selected-item-title">${item.name}</div>

      <div class="inventory-selected-item-desc">
        ${item.desc || 'Без описания'}
      </div>

      ${kind === 'consumable'
        ? `
          <div class="inventory-selected-item-bottom">
            <button class="inventory-selected-use-btn" data-use-id="${item.id}">
              ИСПОЛЬЗОВАТЬ
            </button>
          </div>
        `
...
function renderInventoryModal() {
  ...
  try {
    ...
    const selectedStillVisible = selectedInventoryItem
      && visibleEntries.some(entry => entry.id === selectedInventoryItem.id && entry.kind === selectedInventoryItem.kind);

    if (!selectedStillVisible) {
      selectedInventoryItem = visibleEntries[0];
    } else {
      selectedInventoryItem = visibleEntries.find(entry => entry.id === selectedInventoryItem.id && entry.kind === selectedInventoryItem.kind) || visibleEntries[0];
    }

    visibleEntries.forEach(entry => {
      const { item, kind, count, equipped } = entry;
      const isSelected = selectedInventoryItem
        && selectedInventoryItem.id === entry.id
        && selectedInventoryItem.kind === entry.kind;

      const card = document.createElement('div');
      card.className = 'inventory-card' + (isSelected ? ' selected' : '');

      card.innerHTML = `
        ${equipped ? `<div class="inventory-card-corner-badge">НАДЕТО</div>` : ''}
        <div class="inventory-icon">${item.emoji}</div>
        <div class="inventory-card-title">${item.name}</div>
        ${kind === 'consumable' ? `<div class="inventory-card-count">x${count}</div>` : '<div class="inventory-card-count inventory-card-count-empty"></div>'}
        ${kind === 'clothes'
          ? `
            <button
              class="inventory-quick-btn ${equipped ? 'is-equipped' : ''}"
              type="button"
      ...
      const quickBtn = card.querySelector('.inventory-quick-btn');
      if (quickBtn) {
        quickBtn.addEventListener('click', e => {
          e.stopPropagation();

          if (e.currentTarget.dataset.action === 'unequip') {
            unequipShopItem(e.currentTarget.dataset.itemCategory);
            return;
          }

          equipInventoryItem(e.currentTarget.dataset.itemId);
        });
      }
    ...
    renderInventoryRiderPreview();
    renderInventorySelectedItem();
    requestAnimationFrame(updateInventoryStickyOffsets);
  } catch (error) {
