...
function getInventoryEntries() {
  ...
  const consumables = (player.consumables || [])
    .map(entry => {
      ...
      if (!item || entry.count <= 0) return null;
      return {
        kind: 'consumable',
        id: item.id,
        item,
  ...
  return allEntries.filter(entry => {
    ...
    if (currentInventoryFilter === 'equipment') return entry.kind === 'clothes' && entry.equipped;
...
function getInventoryTypeLabel(entry) {
  ...
  return entry.equipped ? 'ЭКИПИРОВКА' : 'ОДЕЖДА';
}

function getInventoryStatusLabel(entry) {
  if (!entry) return '';
  if (entry.kind === 'consumable') return `x${entry.count}`;
  return entry.equipped ? 'НАДЕТО' : 'В ИНВЕНТАРЕ';
}

function renderInventoryHeaderBalance() {
  const balanceEl = document.getElementById('inventoryCoinsAmount');
  if (balanceEl) {
    balanceEl.textContent = player.coins;
  }
}
...
function renderInventoryRiderPreview(selectedEntry = null) {
  ...
  const headItem = shopItems.find(i => i.id === player.equipped?.head);
  const bodyItem = shopItems.find(i => i.id === player.equipped?.body);
  const selectedEmoji = selectedEntry?.item?.emoji || '';
  const selectedName = selectedEntry?.item?.name || 'Предмет не выбран';
  const riderName = player.name || 'РАЙДЕР';
  const riderType = player.type === 'skate' ? 'СКЕЙТ' : player.type === 'roller' ? 'РОЛИКИ' : 'САМОКАТ';

  preview.innerHTML = `
    <div class="inventory-rider-stage">
      <div class="inventory-rider-figure ${selectedEmoji ? 'has-selected-emoji' : ''}">
        <img class="inventory-rider-image" src="${getSkinImageUrl(player.type, player.skin)}" alt="${riderName}">
        ${selectedEmoji ? `<div class="inventory-rider-selected-emoji">${selectedEmoji}</div>` : ''}
      </div>

      <div class="inventory-rider-meta">
        <div class="inventory-rider-name">${riderName}</div>
        <div class="inventory-rider-subtitle">${riderType}</div>

        <div class="inventory-rider-equipped-row">
          <div class="inventory-rider-chip ${headItem ? 'filled' : ''}">${headItem ? `${headItem.emoji} ${headItem.name}` : 'Без head'}</div>
          <div class="inventory-rider-chip ${bodyItem ? 'filled' : ''}">${bodyItem ? `${bodyItem.emoji} ${bodyItem.name}` : 'Без body'}</div>
        </div>

        <div class="inventory-rider-focus">${selectedName}</div>
      </div>
    </div>
  `;
}

function renderInventorySelectedItem() {
  const selectedInfo = document.getElementById('inventorySelectedItemInfo');
  if (!selectedInfo) return;

  if (!selectedInventoryItem) {
    selectedInfo.innerHTML = `
      <div class="inventory-selected-item-empty">Выбери предмет внизу</div>
    `;
    return;
  }

  const entry = selectedInventoryItem;
  const item = entry.item;
  const statusText = getInventoryStatusLabel(entry);
  const typeText = getInventoryTypeLabel(entry);

  let actionText = 'НАДЕТЬ';
  let actionAttr = `data-action="equip" data-item-id="${item.id}"`;

  if (entry.kind === 'consumable') {
    actionText = 'ИСПОЛЬЗОВАТЬ';
    actionAttr = `data-action="use" data-item-id="${item.id}"`;
  } else if (entry.equipped) {
    actionText = 'СНЯТЬ';
    actionAttr = `data-action="unequip" data-category="${item.category}"`;
  }

  selectedInfo.innerHTML = `
    <div class="inventory-selected-item-card">
      <div class="inventory-selected-topline">
        <div class="inventory-selected-type">${typeText}</div>
        <div class="inventory-selected-status">${statusText}</div>
      </div>

      <div class="inventory-selected-title-row">
        <div class="inventory-selected-icon">${item.emoji}</div>
        <div class="inventory-selected-title-wrap">
          <div class="inventory-selected-title">${item.name}</div>
          <div class="inventory-selected-category">${item.category ? item.category.toUpperCase() : 'ITEM'}</div>
        </div>
      </div>

      <div class="inventory-selected-desc">${item.desc || 'Без описания'}</div>

      <button class="inventory-btn inventory-selected-btn" ${actionAttr}>${actionText}</button>
    </div>
  `;

  const actionBtn = selectedInfo.querySelector('.inventory-selected-btn');
  if (!actionBtn) return;

  actionBtn.addEventListener('click', () => {
    const action = actionBtn.dataset.action;
    if (action === 'equip') {
      equipInventoryItem(actionBtn.dataset.itemId);
      return;
    }
    if (action === 'unequip') {
      unequipShopItem(actionBtn.dataset.category);
      return;
    }
    if (action === 'use') {
      useConsumableItem(actionBtn.dataset.itemId);
    }
  });
}
...
function renderInventoryModal() {
  ...
  try {
    renderInventoryHeaderBalance();
    renderInventoryFilters();
    content.innerHTML = '';

    const visibleEntries = getInventoryEntries();

    if (!visibleEntries.length) {
      selectedInventoryItem = null;
      renderInventoryRiderPreview();
      renderInventorySelectedItem();
      content.innerHTML = `<p class="inventory-empty-state">${(player.ownedShopItems || []).length === 0 && (player.consumables || []).length === 0 ? '🎒 ИНВЕНТАРЬ ПУСТ' : '🎒 НИЧЕГО НЕ НАЙДЕНО'}</p>`;
      return;
    }

    const selectedStillVisible = selectedInventoryItem
      && visibleEntries.some(entry => entry.id === selectedInventoryItem.id);

    if (!selectedStillVisible) {
      selectedInventoryItem = visibleEntries[0];
    } else {
      selectedInventoryItem = visibleEntries.find(entry => entry.id === selectedInventoryItem.id) || visibleEntries[0];
    }

    renderInventoryRiderPreview(selectedInventoryItem);
    renderInventorySelectedItem();

    visibleEntries.forEach(entry => {
      const item = entry.item;
      const isSelected = selectedInventoryItem && selectedInventoryItem.id === entry.id;
      ...
      card.innerHTML = `
        <div class="inventory-card-topline">
          ...
          ${entry.equipped ? '<span class="inventory-badge equipped">НАДЕТО</span>' : ''}
          ${entry.kind === 'consumable' ? `<span class="inventory-badge count">x${entry.count}</span>` : ''}
        </div>
        <div class="inventory-title">${item.name}</div>
      `;

      card.addEventListener('click', () => {
        selectedInventoryItem = entry;
        renderInventoryModal();
      });
