...
function addXP(amount, fromRide = false) {
  ...
  if (fromRide) finalAmount = Math.floor(amount * getRideXPMultiplier());
...
function renderShopModal() {
  ...
  : 0;
    ...
    if (item.type === 'clothes') {
      if (equipped) {
        actionHtml = `<span class="equipped-badge">НАДЕТО</span>`;
      } else if (owned) {
        actionHtml = `<button class="equip-btn" data-id="${item.id}">НАДЕТЬ</button>`;
      } else {
        actionHtml = `<button class="buy-btn" data-id="${item.id}">КУПИТЬ</button>`;
      }
    }
    ...
    if (item.type === 'consumable') {
      actionHtml = `
        <div style="display:flex; flex-direction:column; gap:6px; align-items:flex-end;">
          <span>В сумке: ${consumableCount}</span>
          <button class="buy-btn" data-id="${item.id}">КУПИТЬ</button>
          ${consumableCount > 0 ? `<button class="equip-btn" data-use-id="${item.id}">ИСПОЛЬЗОВАТЬ</button>` : ''}
        </div>
      `;
    }
    ...
    div.innerHTML = `
      ...
      <div>
        <span style="margin-right:12px;">💰${item.price}</span>
        ${actionHtml}
      </div>
    `;
  ...
  content.querySelectorAll('[data-id]').forEach(btn => {
    if (btn.classList.contains('buy-btn')) {
      btn.addEventListener('click', e => buyShopItem(e.currentTarget.dataset.id));
    }
    if (btn.classList.contains('equip-btn')) {
      btn.addEventListener('click', e => equipShopItem(e.currentTarget.dataset.id));
    }
  });

  content.querySelectorAll('[data-use-id]').forEach(btn => {
    btn.addEventListener('click', e => useConsumableItem(e.currentTarget.dataset.useId));
  });
}
...
function equipShopItem(id) {
  ...
  player.equipped[item.category] = id;
...
function openInventoryModal(openModal = true) {
  ...
  if (ownedClothes.length === 0 && consumables.length === 0) {
      ...
      div.innerHTML = `
        ...
        <div>
          ${equipped
            ...
            : `<button class="equip-btn" data-id="${item.id}" data-cat="${item.category}">НАДЕТЬ</button>`}
        </div>
      `;
      content.appendChild(div);
    });
    ...
    consumables.forEach(entry => {
      ...
      div.innerHTML = `
        ...
        <div>
          <span class="equipped-badge">x${entry.count}</span>
          <button class="equip-btn" data-use-id="${item.id}">ИСПОЛЬЗОВАТЬ</button>
        </div>
      `;
      content.appendChild(div);
    });
  ...
  content.querySelectorAll('[data-cat]').forEach(btn => {
    btn.addEventListener('click', e => {
      ...
      player.equipped[button.dataset.cat] = button.dataset.id;
      applyEquipmentBonuses();
      updateStats();
      saveState();
  ...
  if (openModal) openPanelModal('inventoryModal');
}
...
function updateVehiclePreview() {
  ...
  if (baseLayer) baseLayer.src = 'assets/custom/scooter/base/default.png';
  if (deckLayer) deckLayer.src = `assets/custom/scooter/deck/${c.deck}.png`;
  if (barLayer) barLayer.src = `assets/custom/scooter/bar/${c.bar}.png`;
  if (gripsLayer) gripsLayer.src = `assets/custom/scooter/grips/${c.grips}.png`;
  if (headsetLayer) headsetLayer.src = `assets/custom/scooter/headset/${c.headset}.png`;
  if (wheelsLayer) wheelsLayer.src = `assets/custom/scooter/wheels/${c.wheels}.png`;
  if (gripLayer) gripLayer.src = `assets/custom/scooter/grip/${c.grip}.png`;
  if (stickerLayer) stickerLayer.src = `assets/custom/scooter/sticker/${c.sticker}.png`;
}
    ...
    card.innerHTML = `
      ...
      ${
        equipped
          ? '<span class="equipped-badge">НАДЕТО</span>'
          : owned
            ? `<button data-equip-id="${item.id}">НАДЕТЬ</button>`
            : `<button data-buy-id="${item.id}">КУПИТЬ</button>`
      }
    `;
...
function claimDailyBonus() {
  const today = getLocalDateString();
  if (player.lastLoginDate !== today) checkDailyBonus();
...
function checkQuests() {
  ...
  questDefinitions.forEach(quest => {
    ...
    if (quest.rewardCoins > 0) player.coins += quest.rewardCoins;
    if (quest.rewardXP > 0) player.xp += quest.rewardXP;
...
function checkAchievements() {
  ...
  achievements.forEach(ach => {
    ...
    if (ach.rewardCoins > 0) player.coins += ach.rewardCoins;
    if (ach.rewardXP > 0) player.xp += ach.rewardXP;
...
function setupMainEvents() {
  ...
  document.getElementById('repairBtn').addEventListener('click', repairScooter);
  document.getElementById('repairWithXPBtn').addEventListener('click', repairWithXP);
  document.getElementById('claimDailyBonusBtn').addEventListener('click', claimDailyBonus);

  const invBtn = document.createElement('button');
  invBtn.className = 'action-btn';
  invBtn.innerHTML = '<img src="https://img.icons8.com/color/96/t-shirt.png" alt="">Инвентарь';
  invBtn.setAttribute('data-tooltip', 'Экипировка даёт бонусы');
  invBtn.addEventListener('click', () => openInventoryModal(true));
  document.getElementById('gameActions').appendChild(invBtn);
  ...
  miniGame.addEventListener('click', () => {
    ...
    player.scooter = Math.max(0, player.scooter - 10);
