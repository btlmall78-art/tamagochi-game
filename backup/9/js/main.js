...
btn.addEventListener('click', () => {
  ...
  selectedShopItem = null; // 👉 сброс выбора
  renderShopModal();
});
...
card.innerHTML = `
  <div class="shop-item-card-inner">
    <div class="shop-item-icon">${item.emoji}</div>

    <div class="shop-item-title">${item.name}</div>

    <div class="shop-item-type">
      ${item.type === 'clothes' ? 'ОДЕЖДА' : 'РАСХОДНИК'}
    </div>
    ...
    <div class="shop-item-footer">
      ...
      ${item.type === 'consumable'
        ? `<div class="shop-item-count">x${consumableCount}</div>`
        : ''
      }
    </div>
  </div>
...
if (!selectedShopItem) {
  const firstItem = shopItems.find(item => {
    const owned = player.ownedShopItems && player.ownedShopItems.includes(item.id);
...
function renderShopPreview() {
  ...
  if (!selectedShopItem) {
    preview.innerHTML = '<div style="text-align:center; padding:20px;">Выбери предмет</div>';
    return;
  }

  const item = selectedShopItem;

  const owned = player.ownedShopItems?.includes(item.id);
  ...
  if (item.type === 'clothes') {
    status = owned ? 'КУПЛЕНО' : 'НЕ КУПЛЕНО';
  } else {
    status = `x${consumableCount}`;
  }
  ...
  preview.innerHTML = `
    ...
    <div class="preview-price ${notEnough ? 'not-enough' : ''}">
      💰 ${item.price}
    </div>
  ...
  if (btn) {
    btn.addEventListener('click', () => {
buyShopItem(item.id);
// 👉 если это одежда — сброс (она исчезнет)
if (item.type === 'clothes') {
