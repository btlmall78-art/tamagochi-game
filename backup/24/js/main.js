...
function syncInventorySelection(entries) {
  ...
  const exists = safeEntries.find(entry => entry.key === selectedInventoryKey);
  if (exists) return exists;

  selectedInventoryKey = safeEntries[0].key;
...
function renderInventoryModal() {
  ...
  try {
    ...
    visibleEntries.forEach(entry => {
      ...
      div.type = 'button';
      div.className = 'inventory-card' + (selectedInventoryKey === entry.key ? ' selected' : '');
    ...
    content.querySelectorAll('[data-select-key]').forEach(card => {
      card.addEventListener('click', e => {
        selectedInventoryKey = e.currentTarget.dataset.selectKey;
        renderInventoryModal();
      });
    });
