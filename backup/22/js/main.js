...
const shopItems = [
  ...
  },
  {
    ...
    emoji: '🎿',
    price: 28,
    bonus: { driveBonus: 0.05 },
    desc: '+5% драйва за катку'
  },
  {
    id: 'hoodie',
    type: 'clothes',
  ...
  },
  {
    ...
    emoji: '🦺',
    price: 42,
    bonus: { repairDiscount: 0.1 },
    desc: '-10% цена ремонта'
  },
  {
    id: 'backpack',
    type: 'clothes',
    ...
    emoji: '🎒',
    price: 55,
    bonus: { coinsBonus: 0.05, repBonus: 0.05 },
    desc: '+5% монет и репутации'
  },
  {
    id: 'gloves',
    type: 'clothes',
    ...
    emoji: '🧤',
    price: 32,
    bonus: { trickEnergyDiscount: 0.1 },
    desc: '-10% расход энергии на трюки'
  },
  {
    id: 'energy_drink',
    type: 'consumable',
  ...
  },
  {
    ...
    emoji: '🍫',
    price: 18,
    effect: { energy: 18, drive: 8 },
    desc: 'Немного энергии и драйва'
  },
  {
    id: 'isotonic',
    type: 'consumable',
    ...
    emoji: '🧴',
    price: 20,
    effect: { energy: 12, drive: 18 },
    desc: 'Восстанавливает после катки'
  }
];
const workshopItems = [
  {
...
function getInventoryEntries() {
  ...
  return [...ownedClothes, ...consumableEntries];
}
...
function renderInventoryRiderPreview() {
  ...
  const chipsHtml = equippedItems.length
    ? equippedItems.map(item => `<span class="inventory-rider-chip">${item.emoji} ${item.name}</span>`).join('')
    : '<span class="inventory-rider-chip muted">Ничего не надето</span>';

  preview.innerHTML = `
    <div class="inventory-rider-stage">
      <img
        class="inventory-rider-image"
        src="${getSkinImageUrl(player.type, player.skin)}"
        alt="Райдер"
        onerror="this.onerror=null;this.src='${getFallbackDataUri('RIDER')}';"
      >
    </div>
    <div class="inventory-rider-chips">${chipsHtml}</div>
  `;
}
...
function renderInventorySelectedItemInfo() {
  ...
  const currentEntry = entries.find(entry => selectedInventoryItem && entry.item.id === selectedInventoryItem.id);

  if (!currentEntry) {
    selectedInventoryItem = entries[0]?.item || null;
  }

  const entry = entries.find(item => selectedInventoryItem && item.item.id === selectedInventoryItem.id);
  ...
  if (entry.kind === 'consumable') {
    info.innerHTML = `
      <div class="inventory-selected-card">
        <div class="inventory-selected-title-row inventory-selected-title-row-single">
          <div class="inventory-selected-title">${item.name}</div>
        </div>
        <div class="inventory-selected-desc">• ${item.desc || 'Без описания'} •</div>
        <button class="inventory-selected-action" data-use-id="${item.id}">
          ИСПОЛЬЗОВАТЬ
        </button>
      </div>
    `;
  ...
  const toggleBtn = info.querySelector('[data-toggle-id]');
  if (toggleBtn) {
    toggleBtn.addEventListener('click', (e) => {
      e.stopPropagation();
      const toggleItem = shopItems.find(shopItem => shopItem.id === e.currentTarget.dataset.toggleId);
      if (!toggleItem) return;
      const isEquipped = !!(player.equipped && player.equipped[toggleItem.category] === toggleItem.id);
      if (isEquipped) {
        unequipShopItem(toggleItem.category);
      } else {
        equipInventoryItem(toggleItem.id);
      }
    });
  }
...
function renderInventoryModal() {
  ...
  try {
    ...
    const selectedStillVisible = selectedInventoryItem
      && entries.some(entry => entry.item.id === selectedInventoryItem.id);

    if (!selectedStillVisible) {
      selectedInventoryItem = entries[0].item;
    }
    ...
    entries.forEach(entry => {
      const item = entry.item;
      const isSelected = !!(selectedInventoryItem && selectedInventoryItem.id === item.id);

      const div = document.createElement('div');
      div.className = 'inventory-card' + (isSelected ? ' selected' : '');

      const badgeHtml = entry.kind === 'consumable'
        ? `<span class="inventory-card-badge">x${entry.count}</span>`
        : (entry.equipped ? `<span class="inventory-card-badge equipped">НАДЕТО</span>` : '');

      const quickActionHtml = entry.kind === 'clothes' && !isSelected
        ? `<button class="inventory-card-action" data-card-toggle-id="${item.id}">
            ${entry.equipped ? 'СНЯТЬ' : 'НАДЕТЬ'}
          </button>`
        : '';
      ...
      div.innerHTML = `
        ...
        <div class="inventory-desc">${item.desc || 'Без описания'}</div>
        ${quickActionHtml}
      `;
    ...
    content.querySelectorAll('[data-card-toggle-id]').forEach(btn => {
      btn.addEventListener('click', e => {
        e.stopPropagation();
        const item = shopItems.find(shopItem => shopItem.id === e.currentTarget.dataset.cardToggleId);
        if (!item) return;
        const isEquipped = !!(player.equipped && player.equipped[item.category] === item.id);
        if (isEquipped) {
