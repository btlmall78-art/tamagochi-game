...
function renderInventorySelectedItemInfo(entry) {
  ...
  const desc = entry.item.desc || 'Без описания';
  const equipActionMarkup = entry.kind === 'clothes'
    ? `<button class="inventory-selected-action-btn" data-action="${entry.equipped ? 'unequip' : 'equip'}" data-id="${entry.item.id}" data-category="${entry.item.category}">
         ${entry.equipped ? 'СНЯТЬ' : 'НАДЕТЬ'}
       </button>`
    : '';
  const useActionMarkup = entry.kind === 'consumable'
  ...
  info.innerHTML = `
    <div class="inventory-selected-item-card">
      ...
      <div class="inventory-selected-desc inventory-selected-desc-compact">• ${desc} •</div>
      ${useActionMarkup}
    </div>
  `;

  const equipBtn = info.querySelector('[data-action]');
  if (equipBtn) {
    equipBtn.addEventListener('click', (e) => {
      const button = e.currentTarget;
      if (button.dataset.action === 'equip') {
        equipInventoryItem(button.dataset.id);
      } else {
        unequipShopItem(button.dataset.category);
      }
    });
  }
